...
func _ready() -> void:
	...
	chart = FNFChart.parse("Null and Void", "In Your Walls")
	init_note_spawner()
	if chart.assets:
		assets = chart.assets
		load_streams()
	...
	if hud:
		...
		if not skip_countdown:
			Conductor.time = -(5.1 * Conductor.crotchet)
			hud.on_countdown_end.connect(music.play)
			hud.start_countdown()
	if skip_countdown:
		Conductor.time = -(1.5 * Conductor.crotchet)
...
func _unhandled_key_input(event: InputEvent) -> void:
	if not player_strums.has_control: return
	var action: StringName = player_strums.get_action_name(event)
...
func on_note_hit(note: NoteObject) -> void:
	...
	note.was_hit = true
	var abs_diff: float = absf(note.data.time - Conductor.time)
...
func reset_discord_rpc() -> void:
	# this function is stupid
	var pstr := Global.get_paused_string()
	var mstr := Global.get_mode_string(game_mode)
	if pstr.length() != 0: pstr = " (%s)" % pstr
	if mstr.length() != 0: mstr = ", %s" % mstr
	Global.update_discord(
		"Playing %s (%s)" % [ chart.parsed_values.folder, chart.parsed_values.file + mstr ],
		hud.score_text.text,)
